...
router.put('/updatenote/:id', fetchuser, async (req, res) => {
    ...
    try {
        ...
        if (!note) { return res.status(404).send("Not Found"); } // <-- Add closing parenthesis here
        ...
        res.json({ note }); // <-- Corrected typo "res.jsin" to "res.json"
    }
    catch (error) {
        console.error(error.message);
        res.status(500).send("Internal Server Error");
...
router.delete('/deletenote/:id', fetchuser, async (req, res) => {
    try {
        ...
        //Allow the user only if the user owns this Note
        if (note.user.toString() !== req.user.id) {
            return res.status(401).send("Not Allowed");
        }
        ...
        res.json({ "Success": "Note has been deleted", note: note }); // <-- Corrected "res.jsin" to "res.json"
    }
    catch (error) {
        console.error(error.message);
        res.status(500).send("Internal Server Error");
